            ...
            <div className="mt-8 flex flex-col space-y-5">
                {
                    user?.role == 'client' ?
                        dashboardNavLinks.map((navItem, index) => (
                            <a href={navItem.path} key={navItem.index} className={`cursor-pointer flex items-center space-x-3 p-3 rounded-lg ${activeNavIndex === index ? "bg-[#BB9533] text-white rounded-l-xl" : " "}`}>
